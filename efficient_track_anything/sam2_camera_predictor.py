...
# BEFORE: class SAM2CameraPredictor(SAM2Base):
class SAM2CameraPredictor:
    def __init__(
        self,
        sam_model,   # <-- accept the built model
        fill_hole_area=0,
    ...
    def _init_state(
        ...
        # Use whatever device the predictor was initialized with
    ...
    def _manage_memory_obj(self, frame_idx, current_out):
        ...
        if len(non_cond_frame_outputs) > self.model.num_maskmem:
            for t in range(0, len(non_cond_frame_outputs) - self.model.num_maskmem):
    ...
    def _get_feature(self, img, batch_size):
        # 1) Make a float32 tensor on the right device
        if isinstance(img, torch.Tensor):
            image = img.to(self.device, dtype=torch.float32)
        else:
            image = torch.as_tensor(img, device=self.device, dtype=torch.float32)

        # 2) Ensure NCHW (model backbones usually expect this)
        #    If input is HWC or CHW (no batch), add batch dim.
        if image.dim() == 3:  # HWC or CHW
            # If last dim looks like channels, assume HWC -> NCHW
            if image.shape[-1] in (1, 3, 4):
                image = image.permute(2, 0, 1).contiguous()  # CHW
            # Add batch dim
            image = image.unsqueeze(0)  # NCHW

        # 3) Forward image through backbone
        #    (model must already be on self.device)
        backbone_out = self.model.forward_image(image)  # dict with lists of tensors

        # 4) Expand to batch_size (cheap view; no data copy)
        expanded_image = image.expand(batch_size, -1, -1, -1)

        # Shallow copies are fine for the dict structure; we replace list items below
        expanded_backbone_out = {
            "backbone_fpn": list(backbone_out["backbone_fpn"]),
            "vision_pos_enc": list(backbone_out["vision_pos_enc"]),
        }

        for i, feat in enumerate(expanded_backbone_out["backbone_fpn"]):
            # feat: (C,H,W) or (N,C,H,W). Most backbones return (N,C,H,W); if so N=1 here.
            if feat.dim() == 3:
        ...
        # 5) Prepare features for the rest of the pipeline
        features = self.model._prepare_backbone_features(expanded_backbone_out)
        features = (expanded_image,) + features
        return features
...
class SAM2CameraPredictorVOS(SAM2CameraPredictor):
    ...
    def _encode_new_memory(
        ...
        if self.non_overlap_masks_for_mem_enc and not self.training:
            ...
            pred_masks_high_res = self.model._apply_non_overlapping_constraints(
                pred_masks_high_res
            )
