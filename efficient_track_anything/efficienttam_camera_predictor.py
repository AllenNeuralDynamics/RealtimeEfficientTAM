...
class EfficientTAMCameraPredictor(EfficientTAMBase):
    ...
    def load_first_frame(self, img, num_classes=1):
        ...
        self.condition_state["num_classes"] = num_classes
    ...
    def add_conditioning_frame(self, img):
        ...
        self._get_image_feature(
            frame_idx=self.condition_state["num_frames"] - 1, batch_size=self._get_obj_num()
        )
    ...
    def _consolidate_temp_output_across_obj(
        ...
        for obj_idx in range(batch_size):
            ...
            if out is None:
                ...
                # if out is not None: 
                    # print("Got cond results of object ", obj_idx)
                    # print(out)
            if out is None:
                out = obj_output_dict["non_cond_frame_outputs"].get(frame_idx, None)
                # if out is not None: 
                    # print("Got non_cond results of object ", obj_idx)
                    # print(self.condition_state["output_dict_per_obj"][obj_idx]["non_cond_frame_outputs"][frame_idx]["obj_ptr"])
    ...
    def finalize_new_input(
        ...
        self._manage_memory_obj2(self.frame_idx, consolidated_out)
        for obj_temp_output_dict in self.condition_state["temp_output_dict_per_obj"].values():
            obj_temp_output_dict["cond_frame_outputs"].clear()

        for frame_idx in self.condition_state["output_dict"]["cond_frame_outputs"]:
            self.condition_state["output_dict"]["non_cond_frame_outputs"].pop(frame_idx, None)
        for obj_output_dict in self.condition_state["output_dict_per_obj"].values():
            for frame_idx in obj_output_dict["cond_frame_outputs"]:
                obj_output_dict["non_cond_frame_outputs"].pop(frame_idx, None)
        ...
        return self.frame_idx, obj_ids, video_res_masks

    def _manage_memory_obj2(self, frame_idx, consolidated_out):
        output_dict = self.condition_state["output_dict"]
        cond_frame_outputs = output_dict["cond_frame_outputs"]
        self.condition_state["output_dict"]["cond_frame_outputs"][self.frame_idx] = consolidated_out

        key_list = [key for key in output_dict["cond_frame_outputs"]]
        ...
        for obj in range(int(self._get_obj_num())):
    ...
    def _clear_obj_memory(self, frame_idx, obj_id):
        temp_output_dict = self.condition_state["temp_output_dict_per_obj"][obj_id]
        for storage_key in ["cond_frame_outputs", "non_cond_frame_outputs"]:
            for key in temp_output_dict[storage_key].key():
                if key is not frame_idx:
                    temp_output_dict[storage_key].pop(key, None)
        self.condition_state["output_dict_per_obj"][obj_id]["non_cond_frame_outputs"].clear()
        self.condition_state["output_dict_per_obj"][obj_id]["cond_frame_outputs"].clear()

    def _add_output_per_object(self, frame_idx, output, storage_key):
        """
        ...
        for obj_idx, obj_output_dict in output_dict_per_obj.items():
            ...
            obj_out = {
                "maskmem_features": None,
                "maskmem_pos_enc": None,
                "pred_masks": output["pred_masks"][obj_slice],
                "obj_ptr": output["obj_ptr"][obj_slice],
                "object_score_logits": output["object_score_logits"][obj_slice],
            }
            if maskmem_features is not None:
    ...
    def _run_single_frame_inference(
        ...
        current_out = self.track_step(
            ...
            run_mem_encoder = run_mem_encoder and not new_input,
            prev_sam_mask_logits=prev_sam_mask_logits,
            new_input = new_input
        )
